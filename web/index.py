...
from flask import Blueprint, app, current_app, flash, json, make_response, redirect, render_template, request, url_for
import pandas as pd
import urllib
from werkzeug.utils import secure_filename

from core.error import DataExist, InputError
from server.func import success_return
import web.system
import web.webscore
from core.init import FileChecker 
from core.operation import (DeleteUserScore, RefreshAllScoreRating, RefreshBundleCache, RefreshSongFileCache, SaveUpdateScore, UnlockUserItem)
from core.rank import RankList
from core.score import Potential
from core.sql import Connect
from core.user import User, UserLogin, UserRegister
...
@bp.route('/getinvitecode', methods=['GET', 'POST'])
@login_required
def getinvitecode():
    # 生成注册码
    def generate_code(qq_id):
        now = datetime.now()
        ...
        return bytes(i ^ j for i, j in zip(plaintext, xor_key)).hex()
    batch_results = None
    if request.method == 'POST':
        # 单个生成逻辑
        if request.form.get('action') == '单个生成' and request.form.get('qqid'):
            invite_code = generate_code(request.form['qqid'])
            flash(f'注册码生成成功: {invite_code}')
        # 批量生成逻辑
        elif request.form.get('action') == '批量生成':
            import pandas as pd
            from io import BytesIO
            ...
            if excel_file and sheet_name and target_col:
                try:
                    df = pd.read_excel(BytesIO(excel_file.read()), sheet_name=sheet_name)
                    if target_col not in df.columns:
                        flash(f'错误：工作表中不存在 "{target_col}" 列')
                    else:
                        batch_results = {}
                        for qq in df[target_col].dropna():
                            if str(qq).strip():
                                batch_results[str(qq)] = generate_code(qq)
                        
                        flash(f'批量生成完成，共处理 {len(batch_results)} 个QQ号码')
                except Exception as e:
                    flash(f'处理Excel时出错：{str(e)}')
            else:
                flash('请填写所有批量生成所需的字段')
    
    return render_template('web/getinvitecode.html', batch_results=batch_results)

@bp.route('/getchartconstexcel', methods=['GET', 'POST'])
@login_required
def getchartconstexcel():
    if request.method == 'POST':
        try:
            # 读取歌曲数据 - 更新为新的songlist路径
            with open("./database/songs/songlist", "r", encoding="utf-8") as f:
                songs = [{
                    "id": s.get("id"),
                    "set": s.get("set", ""),
            ...
            # 数据库连接 - 更新为新的数据库路径
            with sqlite3.connect("./database/arcaea_database.db") as conn:
                cursor = conn.cursor()
                result = []
                for song in songs:
                    cursor.execute("""
                    ...
                    for diff in song["difficulties"]:
                        diff_info.setdefault(diff["ratingClass"], diff.get("rating", 0))

                    def get_rating(i, data):
                        if diff_info.get(i, 0) != 0:
                            if data is not None and (int(data) != 0 or song["id"] in SP_SPNGS):
                                return data
                            else:
                                return "待填写"
                        else:
                            if data is not None and int(data) != 0:
                                return "该格应为0而不该有定数：" + str(data)
                            else:
                                return 0
            ...
            output.seek(0)
            filename = urllib.parse.quote("Arkana数据库定数表.xlsx")
            ...
            flash(f"定数表生成失败: {str(e)}")
    
    return render_template('web/getchartconstexcel.html')
# --------------------- 以上代码后端代码由SweelLong(Swiro)撰写 ----------
